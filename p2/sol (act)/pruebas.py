...
def ranking(j1, j2, ganador, turno, partida):
    ...
    puntuacion_eliminados_j1 = sum(100 for personaje in jugador.info_eliminados())
    puntuacion_eliminados_j2 = sum(100 for personaje in jugador.info_eliminados())

    # Puntuación por turnos restantes (máximo 200 puntos)
    puntuacion_turnos = max(0, (20 - turno)) * 10

    # Asignación de puntuaciones al ganador y perdedor
    if j1 is ganador:
        puntuacion_ganador += puntuacion_vivos_j1 + puntuacion_eliminados_j1 + puntuacion_turnos
        puntuacion_perdedor += puntuacion_vivos_j2 + puntuacion_eliminados_j2
    elif j2 is ganador:
        puntuacion_ganador += puntuacion_vivos_j2 + puntuacion_eliminados_j2 + puntuacion_turnos
        puntuacion_perdedor += puntuacion_vivos_j1 + puntuacion_eliminados_j1
